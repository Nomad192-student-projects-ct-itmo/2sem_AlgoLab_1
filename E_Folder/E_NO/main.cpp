...
public:
    explicit Tree(size_t n)
    {
        ...
        {
            data[i].sort = new std::multiset <Sub_Element>;
            data[i].left = i - la;
            data[i].right = i - la + 1;
        }
        this->n = n;
...
private:
    ...
    long long rq_search(size_t el, type_tree x, size_t l)
    {
        if(data[el].left == l)
        {
#ifdef _DEBUG
...
#endif
            long long res1 = rq_search(el*2, x, l);
            long long res2 = rq_search(el*2 + 1, x, data[el*2 + 1].left);
            if(res1 == -1)
            {
                return res2;
            }
...
public:
    ...
    void add(type_tree v)
    {
        ...
        data[la + current_add].sort->insert(b);
    ...
    long long search(type_tree x, size_t l)
    {
        if(current_add < n)
        {
            printf("Error: tree is not full\n");
